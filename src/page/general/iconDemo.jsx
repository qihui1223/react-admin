	...
	render(){
		...
		return(
			<div>
				...
				<TypingCard title='图标的命名规范' source={cardContent2} id="standard" height={206}/>
				{
					icons.map((item)=>{
						return( 	
							<Card title={item.title} bordered={false} className="cart-item" 
								key={item.title} id={item.anchor}> 
								{
									item.list.map((icon,index)=>{
										return(
											<Card.Grid style={styles.gridItem} key={index}>
